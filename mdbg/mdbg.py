...
class mdbg_class(object):

    @classmethod
    def translate(cls, word, simplified):
        response_body = cls._get_response(word, simplified)
        return cls._parse_response(response_body)

    @classmethod
    def _get_response(cls, word, simplified):
        res = requests.get(
